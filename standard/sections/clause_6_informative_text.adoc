...
=== The first-mile data collection segment

The first-mile segment covers the connection between field equipment and the systems that collect and process observations. In this specification, the field equipment is represented as the Node and the receiving system as the Hub.

[[fig-first-mile]]
.First-mile data collection segment
[source,text]
----
                   |<--- National segment ------------------->|<-- International -->|
  <-- first mile ->|                                          |      segment        |
                   | first-mile interface      regulated by WMO (WIS)              |
                   | (unregulated)                            |                     |
  +-------------+  | +------------------+  +-----------+     |  +----------------+ |
  | Sensor /    |  | | Data Collection  |  |           |     |  | WIS Global     | |
  | Data logger +--+>| and Processing   +->| WIS2 Node +-----+->| Services       | |
  +-------------+  | | System (DCPS)    |  |           |     |  +----------------+ |
  | Cloud /     |  | +--------+---------+  +-----------+     |                     |
  | Satellite   +--+          |                              |                     |
  | External    |  |          v                              |                     |
...
Communication is strictly unidirectional, with the Node transmitting protobuf payloads over MQTT to the Hub. The two top-level payloads are the `data` message, which carries Observation content, and the `metadata` message, which carries the descriptive context needed to interpret those observations.
